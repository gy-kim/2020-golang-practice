...
// alrreadyExists represents and error when soomething already exists.
...
// NotSupportedf reurns an error which satisfies IsNotSupporteds().
...
// notAssigned represents an error when something is not yet assigned to something else.
...
// NewNotAssigned returns an error which wrap err that satisfies IsNotAssigned().
func NewNotAssigned(err error, msg string) error {
	return &notAssigned{wrap(err, msg, "")}
}

// IsNotAssigned reports whether err was created with NoAssignedf() or NewNotAssigned().
...
// badRequest represents an error when a prequest has bad parameters.
...
// methodNotAllowed rerpresents an error when an HTTP request is made with an inappropricate method.
...
// NewMethodNotAllowed returns an error which wraps err that satisfies IsMethodNowAllowed().
func NewMethodNotAllowed(err error, msg string) error {
	return &methodNotAllowed{wrap(err, msg, "")}
}

// IsMethodNotAllowed reports whether err was created with MethodNotAllowed() or NewMethodNotAllowed().
...
// forbidden represents an error when a request cannot be completed because of missing privileges
type forbidden struct {
	Err
}

// Forbiddenf returns an error whoch satisfies IsForbidden()
func Forbiddenf(format string, args ...interface{}) error {
	return &forbidden{wrap(nil, format, "", args...)}
}

// NewForbidden returns an erro whoch wraps err that satisfies IsForbidden().
func NewForbidden(err error, msg string) error {
	return &forbidden{wrap(err, msg, "")}
}

// IsForbidden reports whether err was created with Forbidden() or NewForbidden().
...
// NewQuotaLimitExceeded returns an error which wraps err and satisfies IsQuotaLimitExceeded.
func NewQuotaLimitExceeded(err error, msg string) error {
	return &quotaLimitExceeded{wrap(err, msg, "")}
}

// IsQuotaLimitExceeded returns true if the given error represents a QuotaLimitExceeded error.
